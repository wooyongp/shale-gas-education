...
dbExecute(con, "
  ...
  SELECT 
    ...
    -- replace NA codings of IPUMS with NULL
    ...
    CASE WHEN INDNAICS LIKE '21%' THEN 1 ELSE 0 END AS oil_and_gas_industry,
    
    -- treat_year_CSDID
    COALESCE(tt.treat_year, 3000) AS treat_year_CSDID,
    
    -- individual_id
    CONCAT(CAST(raw.SERIAL AS VARCHAR), '-', CAST(raw.SAMPLE AS VARCHAR)) AS individual_id,
  ...
  WHERE 
    ...
    AND raw.YEAR <= 2010                 -- Only up to 2010
")
